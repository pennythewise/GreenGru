...
1. **EU CBAM** enters its definitive, financially binding phase from 1 January 2026. Without verified emissions data, these SMEs are hit with punitive default values — real industry reporting puts China-origin defaults at roughly double actual measured emissions, and full-rate CBAM cost on a hex bolt (CN 7318 15 88) reaches an estimated €313/tonne by 2034. **Important nuance confirmed by research (see §6.2a):** the certificate obligation itself is phased in gradually — only 2.5% of taxable emissions require a certificate in 2026, rising to 100% by 2034 — so the *2026* cost on that same hex bolt is a small fraction of the 2034 figure. This is good news for immediate cash exposure but bad news for complacency: the ~40x escalation between 2026 and 2034 is exactly the kind of thing an SME with no in-house carbon expertise will not see coming, which is itself part of the case for this product.
...
3. **Domestic policy pressure.** China's 碳排放总量和强度双控 regime (fully active from 2026) and anchor-enterprise ESG pass-through (e.g. BMW requiring 300+ Tier-1 China suppliers to sign renewable-PPA commitments) mean SMEs face elimination from supply chains even domestically if they can't produce credible carbon data. **Compounding factor confirmed by research:** China's own Ministry of Finance/State Taxation Administration already cancelled export VAT rebates on 169 steel product tariff codes across two 2021 announcements (财政部税务总局公告2021年第16号, effective 1 May 2021, 146 codes; 公告2021年第25号, effective 1 Aug 2021, 23 more codes) — CN 7302 (railway track material, tariff line 73021000) is confirmed on the second list. An SME already absorbing a lost ~13% export VAT rebate on these codes is measurably more exposed to a *new* CBAM cost stacked on top — this is real, citable context for why the squeeze is compounding, not a single new cost in isolation. Do not assume all 8 of this project's CN codes are on the rebate-cancellation list without checking the actual annex per code; only 7302 is confirmed here.
...
|---|---|---|
| Frontend | TanStack Start (Vite, React 19) + TypeScript + Tailwind + shadcn/ui | Fastest path to a professional-looking form + document preview UI within a month; shadcn gives accessible, pre-built components so the frontend person isn't hand-rolling forms. (Originally built on Next.js 14; the frontend was rebuilt on TanStack Start/Vite partway through the build — see repo history — this table was not updated until the two-judge audit caught the drift.) |
...
| Commission Implementing Regulation (EU) 2025/2621 | EU free-allocation benchmarks (BF-BOF 1.370, DRI-EAF 0.481, scrap-EAF 0.072 tCO2e/t) and default-value mark-up (10%/20%/30% for 2026/27/28+, iron & steel is in the same mark-up bracket as cement/aluminium/hydrogen — fertilizer's 1% bracket does not apply here) | Real, confirmed (mark-up figure corroborated across multiple independent 2026 sources — cbamguide.com, co2-iq.com, clcouncil.org — all citing IR 2025/2621 Annex IV point 4.1) |
| **CBAM phase-in factor** — Regulation (EU) 2023/956 Art. 31(3) + Directive 2003/87/EC Art. 10a(1a) (as amended by Directive (EU) 2023/959) | The fraction of taxable emissions actually requiring a certificate each year, mirroring the EU ETS free-allocation phase-out: **2.5% (2026), 5% (2027), 10% (2028), 22.5% (2029), 48.5% (2030), 61% (2031), 73.5% (2032), 86% (2033), 100% (2034+)**. This is *separate from and multiplies with* the default-value mark-up above — see `calculation_engine.py`. **This factor was missing from the original build spec** (the €177/tonne worked example was implicitly the un-phased-in/2034 figure); adding it changes the *current-year* passport number by roughly 10-40x downward for 2026-2033, though the 2034 steady-state number is unchanged. | Real mechanism, confirmed to exist and corroborated across three independent sources including a European Parliament research-service (EPRS) briefing PDF — but the exact percentages are still sourced from secondary reporting, not the delegated act's own formula text. **TODO before launch:** cross-check against Commission Delegated Regulation on the free-allocation-equivalent calculation methodology (referenced by Art. 31(4), not yet directly obtained) |
...
| **PBOC 碳减排支持工具** (Carbon Emission Reduction Facility / CERF) | Central-bank low-cost refinancing: banks get 60% of qualifying loan principal refinanced at 1.75% (well below LPR), passed through as cheaper green loans to the SME. **Expanded January 2026** (announced by PBOC Deputy Governor 邹澜) to explicitly cover 节能改造 (energy-saving retrofits), 绿色升级 (green upgrades), and 能源绿色低碳转型 (energy green-low-carbon transition) — directly applicable to a steel SME's decarbonization capex, not just new clean-energy generation projects | Real, confirmed, current (Jan 2026 expansion announcement) — source: PBOC press briefing via finance.sina.com.cn 2026-01-26 |
| **中小微企业贷款贴息政策** (2026 SME loan interest subsidy) | Central finance pays a 1.5-percentage-point annual interest subsidy on qualifying SME fixed-asset loans (from banks) issued on/after 1 Jan 2026, for up to 2 years, capped at ¥50,000,000 loan principal per borrower. Covers 节能环保服务 (energy-saving/environmental services) among eligible industry chains | Real, confirmed — official State Council document (国务院部门文件, 财政部/MOF, published on gov.cn, Jan 2026) |
...
- **CBAM phase-in factor (§6.2a) is mandatory, not optional.** Compute both `tariff_cost_eur_per_tonne` (net, phase-in adjusted — what's actually owed this year) and `gross_tariff_cost_eur_per_tonne` (as if fully phased in — the 2034 steady-state figure). The passport must show both, clearly labeled, so a small current-year number is never mistaken for the long-run exposure — that misunderstanding is worse than no number at all.
...
12. **Confusing a phase-in-adjusted current-year number with steady-state exposure** — the calculation engine must always emit both `tariff_cost_eur_per_tonne` (net) and `gross_tariff_cost_eur_per_tonne` (§6.2a, §8.4); a test should assert the two values differ for any year before 2034 and are equal from 2034 onward.
...
Fields: company info, CN code + production route, verified/default intensity + data source disclosure, taxable emissions, **both net (phase-in adjusted, current-year) and gross (fully-phased-in, 2034 steady-state) tariff estimates, per-tonne and annual, with the CBAM phase-in factor for the current year shown explicitly** (§8.4), certificate price + quarter used, de minimis status (worded as "possible," not "exempt" — §8.5), anti-circumvention self-declaration, a clarifying note that **the EU importer, not the SME, must hold Authorized CBAM Declarant status** (via an EU-established indirect customs representative if the importer itself isn't EU-established) — the passport is supporting evidence the SME provides to that importer, not a filing the SME makes itself, QR code linking to any public platform verification (e.g. 碳效码) if available, disclaimer line, content hash + signature.
...
- **The CBAM phase-in factor (§6.2a) is a required, permanent part of the calculation engine, not a "nice to have" refinement.** It was discovered missing during a research pass and is now load-bearing: without it, every tariff number this product produces for 2026-2033 is overstated by roughly 10-40x. If a future refactor is tempted to simplify `calculation_engine.py` by dropping the gross/net distinction, that is a correctness regression, not a simplification — re-read `calculation_engine.py`'s own comments before touching this.
...
- Commission Implementing Regulation (EU) 2025/2621 (default values + mark-up, Annex IV point 4.1)
- Regulation (EU) 2023/956 Article 31(3); Directive 2003/87/EC Article 10a(1a) as amended by Directive (EU) 2023/959 — CBAM phase-in factor (§6.2a)
- European Parliament EPRS briefing, "Free allocation in the EU emissions trading system" (2026), europarl.europa.eu — corroborates the phase-in factor schedule
...
- 中国人民银行碳减排支持工具 (PBOC Carbon Emission Reduction Facility) — 2026-01-26 expansion announcement, via finance.sina.com.cn
- 国务院/财政部, 关于实施中小微企业贷款贴息政策的通知 (2026 SME loan interest subsidy notice), gov.cn
- 财政部 税务总局公告2021年第16号 and 公告2021年第25号 — cancellation of export VAT rebates on 169 steel product tariff codes (CN 7302/73021000 confirmed on the second list)
