...
export const newPage = {
  ...
  intakeNote: {
    en: "Stage 1 intake uses chineseocr for photos; PDFs extract text and embed with Qwen text-embedding-v4 into Supabase. Missing fields fall back to cited mock invoice templates. Use the chevron on each row to collapse long lists.",
    zh: "阶段 1 接入：照片走 chineseocr；PDF 提取文本并用 Qwen text-embedding-v4 写入 Supabase。缺失字段回退至引用模板。长列表可用每行箭头折叠。",
  },
...
export const invoiceCard = {
  ...
  intakeDetail: { en: "chineseocr → field parse → CN classify · PDFs also embed via text-embedding-v4", zh: "chineseocr → 字段解析 → CN 分类 · PDF 亦经 text-embedding-v4 嵌入" },
